...
class LLMService:
    ...
    async def enrich_certificate(
        ...
        Args:
            ...
            search_context: Brave 검색 결과를 포맷한 문자열.
        Returns:
...
검색 결과(Brave Search)에서 정확한 정보만 추출하세요. 정보가 없으면 빈 값으로 두세요.
...
"""
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                last_error = e
                if attempt < MAX_RETRIES:
                    print(f"    [Phase 1] 오류 발생, 재시도 중... ({attempt + 2}/{MAX_RETRIES + 1})")
                    continue
                raise last_error
...
**중요: 모든 문자열 값은 유효한 JSON 형식이어야 합니다. 줄바꿈은 \\n을 사용하세요.**"""
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                last_error = e
                if attempt < MAX_RETRIES:
                    print(f"    [Phase 2] 오류 발생, 재시도 중... ({attempt + 2}/{MAX_RETRIES + 1})")
                    continue
                raise last_error
