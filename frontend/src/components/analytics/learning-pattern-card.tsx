    ...
    <Card className="bg-gradient-to-br from-card to-muted border-border overflow-hidden">
      ...
      <CardHeader className="relative">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <p className="text-sm text-muted-foreground mb-1">학습 패턴 분석</p>
      ...
      <CardContent className="relative space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {pattern.preferred_time_slots.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">데이터를 수집 중입니다</p>
            )}
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-muted/50 border border-border">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">평균 학습 시간</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {pattern.best_weekday && (
              <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                <p className="text-xs text-muted-foreground mb-1">가장 효율적</p>
            ...
            {pattern.worst_weekday && (
              <div className="p-3 rounded-lg bg-border/50 border border-border">
                <p className="text-xs text-muted-foreground mb-1">개선 필요</p>
                <p className="text-lg font-semibold text-foreground/80">{pattern.worst_weekday}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {pattern.recent_moods.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-sm font-medium text-foreground/80">최근 7일 기분 추이</h4>
        ...
        <div className="p-4 rounded-xl bg-gradient-to-r from-violet-500/10 to-cyan-500/10 border border-violet-500/20">
          <h4 className="text-sm font-medium text-foreground mb-2">💡 인사이트</h4>
