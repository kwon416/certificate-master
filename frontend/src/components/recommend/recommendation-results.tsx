    ...
    <div className="max-w-5xl mx-auto px-3 py-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-0.5 md:space-y-1">
          <h2 className="text-2xl md:text-3xl font-bold text-white">추천 결과</h2>
      ...
      {userSummary && (
        <div className="rounded-xl md:rounded-2xl border border-emerald-500/30 bg-emerald-950/20 p-3 md:p-4">
      ...
      {querySummary && (
        <div className="rounded-xl md:rounded-2xl border border-slate-800/70 bg-slate-900/70 p-3 md:p-4">
          <p className="text-xs md:text-sm font-semibold text-emerald-200 mb-2 md:mb-3">검색 조건</p>
          <div className="flex flex-wrap gap-1.5 md:gap-2">
            {parseSummary(querySummary).map((item, index) => (
              <div
                key={index}
                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 md:px-3 md:py-2 rounded-lg bg-slate-800/80 border border-slate-700/50"
              >
      ...
      {normalized.length === 0 ? (
        <div className="rounded-xl md:rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 md:p-8 text-center">
          <p className="text-slate-300 mb-2 md:mb-3 text-sm md:text-base">조건에 맞는 추천이 없습니다.</p>
      ...
      <div className="mt-6 md:mt-12 text-center pb-4">
        <p className="text-slate-400 mb-3 md:mb-4 text-sm md:text-base">다른 추천을 받고 싶으신가요?</p>
